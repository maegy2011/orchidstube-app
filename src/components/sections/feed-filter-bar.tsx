...
const FeedFilterBar = memo(function FeedFilterBar({ onCategoryChange }: { onCategoryChange?: (category: string) => void }) {
  ...
  let stickyTop: string;
  if (isRamadanCountdownVisible && isPrayerBarVisible) {
    stickyTop = 'top-[144px] sm:top-[136px]';
  } else if (isRamadanCountdownVisible || isPrayerBarVisible) {
    stickyTop = 'top-[104px] sm:top-[100px]';
  } else {
    stickyTop = 'top-[64px]';
  }

  return (
    <div className={`sticky ${stickyTop} z-[8500] w-full bg-gradient-to-b from-background via-background/95 to-background/90 backdrop-blur-md px-4 flex items-center h-[56px] border-b border-border/50 shadow-sm transition-all duration-300`}>
