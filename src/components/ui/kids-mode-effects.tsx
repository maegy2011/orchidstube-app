...
function Particle({ data, mouseX, mouseY, scrollY }: { 
  ...
  const tx = useSpring(useTransform(mouseX, (val: number) => (val - (typeof window !== 'undefined' ? window.innerWidth / 2 : 0)) * data.parallaxFactor * 0.1), springConfig);
  const ty = useSpring(useTransform(mouseY, (val: number) => (val - (typeof window !== 'undefined' ? window.innerHeight / 2 : 0)) * data.parallaxFactor * 0.1), springConfig);
  const sy = useTransform(scrollY, (val: number) => val * data.parallaxFactor * -0.5);
