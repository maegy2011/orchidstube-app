...
export default function ContentFilterAdminPage() {
  ...
  const toggleFilterEnabled = async (enabled: boolean) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('Filter update failed:', res.status, res.statusText, errorText);
  ...
  const toggleDefaultDeny = async (defaultDeny: boolean) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text().catch(() => '');
        console.error('Default deny update failed:', res.status, res.statusText, errorText);
        toast.error(`فشل التحديث (${res.status})`);
        setSaving(false);
        return;
      }
      ...
      const data = await res.json();
      console.log('Default deny update response:', data);
      ...
      console.error('Toggle default deny error:', error);
      toast.error('حدث خطأ أثناء تحديث الإعدادات');
    } finally {
      setSaving(false);
    }
