...
# Fix: .config may be a JuiceFS storage file, not a directory.
# Prisma expects .config/prisma to be a directory path.
# Temporarily relocate .config if it's a file.
_CONFIG_MOVED=false
if [ -f "$PROJECT_DIR/.config" ] && [ ! -d "$PROJECT_DIR/.config" ]; then
        echo "[BUN] .config is a file (likely JuiceFS), temporarily moving..."
        mv "$PROJECT_DIR/.config" "$PROJECT_DIR/.config.juicefs.bak"
        _CONFIG_MOVED=true
fi

bun run db:push

if [ "$_CONFIG_MOVED" = true ]; then
        echo "[BUN] Restoring .config file..."
        mv "$PROJECT_DIR/.config.juicefs.bak" "$PROJECT_DIR/.config"
fi
